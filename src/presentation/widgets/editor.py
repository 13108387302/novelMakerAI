...
class DocumentTab(QWidget):
    ...
    def _setup_ai_panel(self):
        ...
        try:
            ...
            self.ai_panel = DocumentAIPanel(self.document.id, document_type, self.ai_assistant, self)
    ...
    def _insert_ai_text(self, text: str):
        """插入AI生成的文本"""
        try:
            cursor = self.text_edit.textCursor()
            cursor.insertText(text)
            self.text_edit.setTextCursor(cursor)
            logger.debug(f"插入AI文本: {len(text)} 字符")
        except Exception as e:
            logger.error(f"插入AI文本失败: {e}")

    def _replace_ai_text(self, text: str):
        """替换选中的文本为AI生成的文本"""
        try:
            cursor = self.text_edit.textCursor()
            if cursor.hasSelection():
                cursor.insertText(text)
                self.text_edit.setTextCursor(cursor)
                logger.debug(f"替换AI文本: {len(text)} 字符")
            else:
                # 如果没有选择，则插入
    ...
    def load_document(self, document: Document):
        ...
        try:
            ...
            if self.ai_assistant_manager:
                self._create_ai_assistant_async(document.id, tab)
