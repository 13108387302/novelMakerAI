...
from src.presentation.widgets.global_ai_panel import ComprehensiveGlobalAIPanel
...
from src.presentation.widgets.document_ai_panel import DocumentAIPanel
...
class MainWindow(QMainWindow):
    ...
    def _create_widgets(self):
        ...
        # 全局AI面板
        if self.controller and hasattr(self.controller, 'ai_service'):
            self.global_ai_panel = ComprehensiveGlobalAIPanel(self.controller.ai_service)
        else:
            from PyQt6.QtWidgets import QLabel
            self.global_ai_panel = QLabel("AI服务暂不可用")
        ...
        try:
            if document and self.controller and hasattr(self.controller, 'ai_assistant_manager'):
                ...
                if ai_assistant_manager:
                    ...
                    # 创建文档AI面板
                    from src.presentation.widgets.document_ai_panel import DocumentAIPanel
                    document_type = str(document.document_type).split('.')[-1] if hasattr(document, 'document_type') else 'chapter'

                    new_ai_panel = DocumentAIPanel(document.id, document_type, ai_assistant)
    ...
    def _show_ai_config(self):
        """显示AI配置对话框"""
        try:
            from src.presentation.dialogs.ai_config_dialog import AIConfigDialog
            dialog = AIConfigDialog(self.controller.settings_service, self)
            dialog.exec()
        except Exception as e:
            logger.error(f"显示AI配置对话框失败: {e}")
            QMessageBox.warning(self, "错误", f"无法打开AI配置对话框: {e}")
